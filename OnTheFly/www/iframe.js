...
//CHANGE TITLE WHEN SIDEBAR COLLAPSES
var count = 0;
$('.sidebar-toggle').on('click', function () {
  count++
  if (count % 2 != 0) {
    $('.logo').html('<img = src="https://icon-library.com/images/dna-helix-icon/dna-helix-icon-7.jpg"></img>');
    $('.logo').parent().find('img').css({
      'width':'52px',
      'margin-left':'-16px'
  });
  }
  else {
    $('.logo').text('OnTheFly2.0');
  }
});
