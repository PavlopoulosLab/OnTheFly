    ...
    for (i in 1:nrow(upload)) {
      ...
      {#-resample 72
        ...
        if( res_x <150 | res_y <150)
        {
          ...
          system(sprintf("ocrmypdf --output-type pdf --remove-vectors --threshold www/tmp/%s.png www/tmp/%s-OCR.pdf", id, id))
      ...
      else if (tolower(ext) %in% post_exts)
      {
        ...
        print(sprintf("ps2pdf www/tmp/%s.%s www/tmp/%s.pdf", id, ext, id))
        Sys.sleep(10)
