  ...
  if(length(file_ids)<max_files & nrow(upload)<max_files)
  {
    if(length(file_ids)>0)
    {
      ...
      for (i in 1:length(file_ids))
      {
        ...
        {
          print("Seems to be from an old session. Will not keep it")
        }
      }
      if(length(chk)>0)
      {
    ...
    for (i in 1:nrow(upload)) {
      ...
      {#-resample 72
        ...
        res_x <- as.integer(img_info[[1]][1])
        res_y <- as.integer(img_info[[1]][2])
        ...
        if( res_x <150 | res_y <150)
        {
          ...
          print("Perform OCR operation...")
          system(sprintf("wsl ocrmypdf --output-type pdf --remove-vectors --threshold www/tmp/%s.png www/tmp/%s-OCR.pdf", id, id))
      ...
      else if (tolower(ext) %in% post_exts)
      {
        ...
        print(sprintf("wsl ps2pdf www/tmp/%s.%s www/tmp/%s.pdf", id, ext, id))
        Sys.sleep(10)
        ...
        system(sprintf('wsl libreoffice --convert-to html --outdir www/tmp www/tmp/%s.%s', id, ext))
      }
      if(error != 1)
      {
        #file_ids[[start+i]] <<- toString(id)
        #file_names[[start+i]] <<- upload$name[[i]]    
...
txt.file_new <- function (textinput, session) {
  ...
  if(length(file_ids)<max_files)
  {
    if (textinput != "") {
      ...
      #file_ids[[start+1]] <<- toString(id)
      #file_names[[start+1]] <<- sprintf('custom_text-%s', toString(timeid))
      file_ids<<-append(toString(id), file_ids)
...
csv.entities <- function (entities) {
  ...
  if(csv != '') {
    ...
    {
      #print(sprintf("%s %s %s", fcsv$V1[i], fcsv$V2[i], fcsv$V3[i]))
      if(fcsv$V3[i] %in% ent_seen_ids)
      {
        #print(uniq_df$X1[uniq_df$X3==as.character(fcsv$V3[i])])
    ...
    datatable(datatable,
              ...
              options = list(
                ...
                columnDefs = list(list(className = 'dt-center', targets = c(1,2,3,4,5,6)),
                                  list(visible=F, targets=c(0,8,9)),
                                  list(width="40px", targets=c(1)),
                                  list(width="100px", targets=c(2)),
                                  list(width="20px", targets=c(3)),
                                  list(width="20px", targets=c(4)),
...
handleBarPlot <- function(DB_source, sliderBarplot, barplotMode, from_barPlotSelect, output, session){
  ...
  if (!identical(DB_source, "")){
    ...
    gostres_m <- data_table[grepl(DB_source,data_table$Source),]
    ...
    #output$barplot_download_button <- renderUI({actionBttn("download_barplot", label="Download",  style = 'material-flat', color="success", size = 'md', icon = icon('save'))})
    
    # Check mode of execution
    if(barplotMode == "-log10(P-value)"){
      gostres_sort <- gostres_m[order(gostres_m[["-log10(P-value)"]], decreasing = T),]
      drawBarplot(gostres_sort[["-log10(P-value)"]], gostres_sort[["Term Name"]], new_color, "P_VALUES BARPLOT", DB_source, sliderBarplot, output)
      tbl_out <- gostres_sort[, c(1,2,3,4,9,5,6,7,8)]
      
      
      #download_bar(gostres_m[["-log10(P-value)"]], gostres_m[["Term Name"]], DB_source, new_color, "P_VALUES BARPLOT", sliderBarplot, output)
    }
    else { # Enrichment Mode
      gostres_sort <- gostres_m[order(gostres_m[["Enrichment Score"]], decreasing = T),]
      drawBarplot(gostres_sort[["Enrichment Score"]], gostres_sort[["Term Name"]], new_color, "ENRICHMENT SCORE", DB_source, sliderBarplot, output)
...
# @param score: enrichment score or pvalue score
# @param terms: names of functions to be drawn
# @param color: color based on manhattan datasource colors
# @param mode: string: "P_VALUES BARPLOT" or "ENRICHMENT SCORE BARPLOT"
# @param sliderBarplot: integer value for cutoff from slider
# @param DB_source: chosen eligible datasource (from executed gost)
# @return void
drawBarplot<- function(score, terms, color, mode, DB_source, sliderBarplot, output){  #p_value_MF$P_VALUE,p_value_MF$FUNCTION
  pdf(NULL)
  output$barplot <- renderUI({
    plotlyOutput("barplot1", height = height_barplot(sliderBarplot))
  })
  ax <- as.numeric(unlist(score[1:sliderBarplot]))
  ay <- as.character(unlist(terms[1:sliderBarplot]))
  data <- data.frame(ax, ay, stringsAsFactors = FALSE)
  ...
  if (mode == "P_VALUES BARPLOT"){
    ...
    bar <- plot_ly(data, x = ~ax, y = ~ay, type = 'bar', orientation = 'h', color=I(color), name = paste(mode, ": ", DB_source, sep = "" ))  %>% layout(title = paste(mode, ": ", DB_source, sep = "" ), xaxis = list(title = "-log10(P-value)"), yaxis=list(title=""))
  } else { # ENRICHMENT SCORE BARPLOT
    par(las=1) # make label text perpendicular to axis
    par(mar=c(5, 30, 2, 10)) # increase y-axis margin.
    bar <- plot_ly(data, x = ~ax, y = ~ay, type = 'bar', orientation="h", color=I(color)) %>% layout(title = paste(mode, ": ", DB_source, sep = "" ), xaxis = list(title = "Enrichment score %"), yaxis=list(title=""))
  }
